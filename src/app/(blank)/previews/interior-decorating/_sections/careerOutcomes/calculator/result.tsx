...
export const Result: FC<Props> = ({ projectCount, projectFee, tier }) => {
  const annual = projectCount * projectFee * (tier === 'standard' ? 1 : 1.25);
  ...
  return (
    <div className="card bg-light text-center">
      <div className="card-body">
        ...
        <p className="display-5 fw-bold text-primary">${formatter.format(annual)} / yr</p>
        <p className="mb-0">Potential Monthly Income</p>
        <p className="display-5 fw-bold text-primary">${formatter.format(monthly)} / month</p>
      </div>
    </div>
  );
};
