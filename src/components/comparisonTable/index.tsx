...
export const ComparisonTable: FC<Props> = <T extends string = string,>({ columns, rows }: Props<T>) => {
  ...
  const hasMobileSwitcher = comparisonColumns.length > 1;
  ...
  return (
    <>
      {hasMobileSwitcher && (
        <div className={`${styles.mobileSwitcher} btn-group rounded-pill p-1 mb-3 d-lg-none`} role="group" aria-label="Choose a comparison column">
          {comparisonColumns.map(column => (
            <button type="button" className={`btn btn-sm rounded-pill fw-semibold ${styles.mobileSwitcherButton}`} aria-pressed={selectedColumnKey === column.key} data-column-key={column.key} key={column.key} onClick={handleMobileSwitcherClick}>{column.mobileLabel}</button>
          ))}
        </div>
      )}
